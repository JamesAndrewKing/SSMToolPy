...
| `SSMTool/src/misc/reduced_to_full_traj.m` | core utility | maps reduced SSM trajectory to full coordinates in Lorenz live script | one-dimensional autonomous Lorenz graph slice implemented in `examples/lorenz_1st_order/lorenz.py` | tested for Lorenz | differentiable for fixed graph coefficients |
...
| `examples/lorenz_1st_order/lorenz.py::lorenz_reduced_trajectory` | linear reduced dynamics prediction stage | differentiable for fixed times | deterministic value/shape tests |
| `examples/lorenz_1st_order/lorenz.py::lorenz_reduced_to_full_trajectory` | reduced-to-full lifting stage | differentiable for fixed graph coefficients | lifting shape/correctness tests |
| `examples/lorenz_1st_order/lorenz.py::lorenz_unstable_ssm_curve` | visualization data assembly for two-sided SSM curve | differentiable for fixed times and coefficients | concatenation/shape tests |
| `examples/lorenz_1st_order/lorenz.py::lorenz_full_unstable_trajectories` | full trajectory validation data assembly | differentiable for fixed times | shape/initial-condition tests |
