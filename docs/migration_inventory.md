...
| Lorenz1stOrder | `build_model.m`, `lorenz.m`, `demo.mlx` | low-medium | canonical first-order nonlinear system | `systems/lorenz.py`, `examples/lorenz_1st_order/example.py` | `examples/lorenz_1st_order/lorenz_1st_order.ipynb` | partial: vector-field, direct trajectory, and fixed-choice unstable SSM graph coefficient subproblems implemented; reduced-to-full mapping and visualization incomplete |
...
| `src/ssmtoolpy/systems/lorenz.py::lorenz_unstable_eigenpair` | setup/classification for one-dimensional unstable SSM | not differentiable due eigenvalue selection and sign normalization | eigenpair regression tests |
| `src/ssmtoolpy/systems/lorenz.py::solve_lorenz_unstable_graph_coefficients` | fixed SSM coefficient computation stage for Lorenz unstable graph | differentiable under fixed eigenpair, fixed order, and nonresonance assumptions | coefficient shape, invariance residual, and JAX grad tests |
| `src/ssmtoolpy/systems/lorenz.py::lorenz_unstable_ssm_graph_coefficients` | combined setup plus fixed coefficient solve | not differentiable as a whole because setup choices are included | coefficient and residual tests |
| `src/ssmtoolpy/systems/lorenz.py::evaluate_lorenz_ssm_graph` | reduced-to-full graph evaluation stage | differentiable for fixed coefficients | shape and linear-term tests |
| `src/ssmtoolpy/systems/lorenz.py::lorenz_ssm_invariance_residual` | fixed-choice SSM graph diagnostic | differentiable for fixed coefficient length and reduced dynamics | invariance residual tests |
